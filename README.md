...
The save is stored locally in the browser. Open `/?preview=mountain` for a 3D Alpe d’Huez preview at a steady 10% gradient, including on the published game. Use `/?preview=descent` to compare a 4% downhill. Both previews run in memory without reading or changing the normal career save; use “Return to career” to leave them. Grades are exaggerated visually, with crests and valleys, while gameplay still uses the real gradient. Development builds also support save-neutral visual-QA query parameters such as `qaFresh=1`, `qaPaused`, `qaStage`, `qaEncounter`, `qaFlow`, and `qaPowerUp` for repeatable screenshots. `qaFresh=1` runs an isolated Level 1 career without reading or changing the normal save.
