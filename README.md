...
The soundtrack and 19 event effects are original deterministic compositions generated by `scripts/generate-audio.mjs`. The checked-in OGG files are used when supported, with MP3 fallbacks; rerun `npm run generate:audio` after changing the score or sound design.
